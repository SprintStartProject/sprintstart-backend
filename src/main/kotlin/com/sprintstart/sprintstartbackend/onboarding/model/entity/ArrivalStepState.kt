 ...
 *
 * The row's existence is the state. There is no status column — the only value it could
 * hold is `SETTLED`. Absence means "not settled yet", never an error, and a hire with no rows at
 * all is one who has just arrived.
 *
 * [stepKey] is a string, not a foreign key to [ArrivalStep.id]. A definition can be deleted,
 ...
 *
 * Monotonic. Settled never becomes unsettled: a derivation that later cannot see its
 ...
 *
 * Rigor lives here, not on the definition. [ArrivalStep.settledBy] says how a step is
