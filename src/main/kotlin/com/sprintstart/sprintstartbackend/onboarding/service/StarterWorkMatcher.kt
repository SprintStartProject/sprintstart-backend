 ...
 *
 * Deterministic and pure, on purpose. Each signal contributes both points *and* the
...
object StarterWorkMatcher {
     ...
     *
     * Review is not a gate: a mined task is claimable the moment it is mined, so this is the only
     * thing expressing "somebody has vouched for this one". It is deliberately smaller than the
     * smallest positive signal ([NEWCOMER_SAFE_TYPE_BONUS]) — an unreviewed task that fits a hire
     * perfectly must still beat a reviewed one that does not, or the demotion has quietly turned
     * itself back into an approval requirement.
     */
