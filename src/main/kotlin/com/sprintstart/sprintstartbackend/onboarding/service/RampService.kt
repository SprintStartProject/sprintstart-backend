...
class RampService(
     ...
     *
     * An engineer reads "You merged your first change here"; a delivery lead reads about
     * facilitated ceremonies. The sentence structure is fixed and only the nouns are supplied,
     * which is the whole reason the vocabulary is structured fields rather than free text.
     */
    private fun unlockedBy(acceptedCount: Int, autonomous: Boolean): String = when {
        autonomous ->
            "You ${ContributionWording.VERB_PAST} a ${ContributionWording.NOUN} with no help and no rework"
        acceptedCount == 0 ->
            "You haven't ${ContributionWording.VERB_PAST} anything here yet — that's the whole first step"
        acceptedCount == 1 ->
            "You ${ContributionWording.VERB_PAST} your first ${ContributionWording.NOUN} here"
        else ->
            "You've ${ContributionWording.VERB_PAST} $acceptedCount ${ContributionWording.NOUN_PLURAL} here"
    }
    ...
    private fun evaluateAutonomy(
        ...
        val latest = accepted.maxByOrNull { it.acceptedAt!! }
            ?: return AutonomyResponse(
                ...
                blockers = listOf(
                    "No ${ContributionWording.VERB_PAST} ${ContributionWording.NOUN} here yet",
                ),
            )
        ...
        if (latest.returnedCount > 0) {
            blockers +=
                "Your last ${ContributionWording.VERB_PAST} ${ContributionWording.NOUN} was sent back for rework"
        }
        if (neededHelp(hireId, projectId, latest)) {
            blockers += "You pulled in a person while you were on your last ${ContributionWording.NOUN}"
        }
