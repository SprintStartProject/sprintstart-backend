...
data class ResolvedArrivalStep(
     ...
     *
     * The client groups under this name, so it must be a name rather than an id — a hire's
 ...
 *
 * Nothing here blocks anything. There is deliberately no method answering "may this hire
 ...
 * method.
 *
 * Uniqueness is enforced here as well as in the database. A step's key must be unique within
...
class ArrivalStepService(
     ...
     *
     * Scoped to *all* the hire's projects rather than one: arrival is a fact about a person,
     * not about a project.
     *
    ...
    fun forHire(userId: UUID): List<ResolvedArrivalStep> {
        ...
        // Ordered by scope, not by position across scopes: positions are assigned *within* a
    ...
    fun confirm(userId: UUID, key: String): ResolvedArrivalStep {
        ...
        // `selfConfirmable` is not a synonym for `settledBy`. Some derived steps are still the
    ...
    fun create(
        ...
        // A key the system knows how to check is always created as a derived step, whatever
     ...
     *
     * The key is not updatable. State points at it, so changing it would orphan every
     ...
     *
     * Takes the complete list rather than a from/to pair, so two people reordering
     ...
     *
     * [ArrivalStepState] is keyed by the step's key rather than by a foreign key, so removing
