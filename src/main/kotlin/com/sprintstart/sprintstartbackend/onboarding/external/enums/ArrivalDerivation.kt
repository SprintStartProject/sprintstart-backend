 ...
 *
 * A closed catalog, not a field on the row. A derivation is code, so a derived step cannot
 ...
 *
 * Nothing here is seeded; an admin adds the ones their organisation wants.
 *
 * Observing settles; failing to observe never refutes. Every derivation answers *"can I see
...
enum class ArrivalDerivation(
     ...
     *
     * Checked rather than trusted: this value is what artifact verification compares a pull
     * request's author against, and a typo silently stops crediting work the hire really did.
     *
     * Not self-confirmable. The check is definitive when it answers, so letting somebody
     ...
     *
     * Derived, never stored separately: a contribution already proves the environment worked.
     *
     * Self-confirmable, and that is the important half. The evidence arrives *late* — by the
