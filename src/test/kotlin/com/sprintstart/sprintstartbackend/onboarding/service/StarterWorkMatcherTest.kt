...
class StarterWorkMatcherTest {
    ...
    fun `competency overlap is one input, not the whole score`() {
        ...
        // Same competency match; the second task wins on the context signals beyond overlap.
    ...
    /**
     * Review is a ranking signal, not a gate, so what has to hold is that it *only* ranks: an
     * unreviewed task is still suggested, and a good fit still wins.
     */
    @Nested
