...
/**
 * Compaction's own rules: fold once the window outgrows the limit, and persist the returned note
 * and the advanced cursor together.
 *
 * These belong here rather than with the agent turn, which no longer folds. Deleting a test along
 * with its subject is how a rule stops being checked without anybody deciding it should, and the
 * diff looks like pure subtraction either way.
 */
class BuddyCompactionServiceTest {
    ...
    /**
     * Catches up in one call rather than one exchange at a time. A pass that missed its turn
    ...
    /**
     * An unavailable model leaves the cursor exactly where it was. Advancing it past messages
    ...
    /**
     * The first of the two races: a fold committed while this one's model call was in flight.
    ...
    /**
     * The second race, and why the entity carries a `@Version` at all: a competing fold can
     * commit *between* the re-read above and the flush, which no re-check can see: narrowing
     * that window is not closing it.
     */
    @Test
