...
class BlueprintPhaseAdminControllerTest : BlueprintControllerTestSupport() {
    ...
    override val controller = BlueprintPhaseAdminController(service, subGraphNodeService)
    override val controllerClass = BlueprintPhaseAdminController::class

    override val endpointCases =
        listOf(
            EndpointCase(
                name = "GET /path/{pathId}/phases",
                request = get("/api/v1/onboarding/blueprints/path/$pathId/phases"),
                expectedStatus = 200,
                response = listOf(phaseResponse),
                serviceCall = {
                    service.getBlueprintPhasesForPath(BlueprintScope.Global, pathId)
                },
                documentsNotFound = false,
            ),
            EndpointCase(
                name = "GET /phase/{phaseId}/graph",
                request = get("/api/v1/onboarding/blueprints/phase/$phaseId/graph"),
                expectedStatus = 200,
                response = GetBlueprintSubGraphResponse(nodes = emptyList()),
                serviceCall = {
                    subGraphNodeService.getSubGraph(BlueprintScope.Global, phaseId)
                },
                documentsNotFound = false,
            ),
            EndpointCase(
                name = "GET /phases/{phaseId}",
                request = get("/api/v1/onboarding/blueprints/phases/$phaseId"),
                expectedStatus = 200,
            ...
            EndpointCase(
                ...
                request =
                    post("/api/v1/onboarding/blueprints/path/$pathId/phases")
                ...
                serviceCall = {
                    service.createBlueprintPhaseForPath(BlueprintScope.Global, pathId, createBody)
                },
            ...
            EndpointCase(
                ...
                request =
                    put("/api/v1/onboarding/blueprints/phases/$phaseId")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(
                ...
                serviceCall = {
                    service.updateBlueprintPhaseById(BlueprintScope.Global, phaseId, updateBody)
                },
                documentsBadRequest = true,
                documentsConflict = true,
            ),
            EndpointCase(
                ...
                request =
                    put("/api/v1/onboarding/blueprints/phases/$phaseId/position")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("""{"revision":0,"position":0}"""),
                expectedStatus = 200,
                response = listOf(UpdateBlueprintPhasePositionResponse(id = phaseId, revision = 0, position = 0)),
                serviceCall = {
                    service.updateBlueprintPhasePositionById(BlueprintScope.Global, phaseId, updatePositionBody)
                },
                documentsBadRequest = true,
                documentsConflict = true,
            ),
            EndpointCase(
                ...
                request =
                    delete("/api/v1/onboarding/blueprints/phases/$phaseId")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("""{"revision":0}"""),
                expectedStatus = 200,
                response = DeleteBlueprintPhaseResponse(updatedPhases = emptyList()),
                serviceCall = {
                    service.deleteBlueprintPhaseById(BlueprintScope.Global, phaseId, deleteBody)
                },
                documentsConflict = true,
            ),
        )
