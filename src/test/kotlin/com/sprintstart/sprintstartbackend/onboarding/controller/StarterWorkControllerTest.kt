...
class StarterWorkControllerTest(
    ...
    fun `listCandidates should return 200 with the pool marking, for a PM`() {
        ...
        mockMvc
            ...
            // Null means "we do not know", not "nobody" -- so it has to reach the client as null
    ...
    fun `getMatchesForMe should return 200 for a USER, with the reasons a task was suggested`() {
        val projectId = UUID.randomUUID()
        // Not a suspend method: ranking is deterministic and local, so there is no AI call to
        // dispatch asynchronously around.
